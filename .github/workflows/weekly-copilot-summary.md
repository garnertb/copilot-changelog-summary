...
echo "Summary week: $WEEK_START_DATE - $WEEK_END_DATE"
...
Also use `web-fetch` to retrieve the VS Code blog RSS feed at `https://code.visualstudio.com/feed.xml` and scan for recent Copilot or AI-related posts that may not appear on the main updates page.
...
## Step 3: Check Repo Memory for Previous Run Data

Before fetching sources, use `repo-memory` to check for data from previous runs this week. Look for a memory entry tracking:
- Which changelog entry URLs were already processed
...
## Step 6: Save Run State to Repo Memory

After publishing, use `repo-memory` to store the current run state with:
- `last_run_date`: today's date
- `week_start`: Monday's date
- `week_end`: Sunday's date
- `processed_urls`: list of changelog entry URLs included in this run
- `vscode_version_checked`: latest stable version checked

This ensures the next run within the same week can quickly identify new entries without reprocessing everything.
